...
def new_loss_fn(y_true, y_pred):
    ...
    loss1_pred = tf.reduce_mean(squared_difference, axis=-1)  # Note the `axis=-1`
    return loss1_pred
...
#         deep_feature_map = input_layer
    ...
    if depth>1:
        ...
        for depth_i in range(1,depth):
            ...
            # Activation
            ################################################################################
            if depth_i == 1: #for depth_i == 1 we are arrived in the finete dimensional measurements space (Koopman)
                core_layers = tf.nn.swish(core_layers) #swish(x) = x * sigmoid(x)
    ...
    opt = opt.apply_gradients(zip(grads, trainable_layers_model.trainable_variables))
    ...
    trainable_layers_model.compile(optimizer=opt, loss=custom_loss)
