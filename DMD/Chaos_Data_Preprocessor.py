...
if Option_Function == "ks":
    ...
    ks_input_data = pd.read_csv('inputs/KS_system/ks_input_data.csv')
    # set index
    ks_input_data.set_index('index', drop=True, inplace=True)
    ks_input_data.index.names = [None]

    # Remove Missing Data
    ks_input_data = ks_input_data[ks_input_data.isna().any(axis=1)==False]
    
    print('ks input data', ks_input_data)
    
    # Read Dataset - get output data
    ks_output_data = pd.read_csv('inputs/KS_system/ks_output_data.csv')
    # set index
    ks_output_data.set_index('index', drop=True, inplace=True)
    ks_output_data.index.names = [None]

    # Remove Missing Data
    ks_output_data = ks_output_data[ks_output_data.isna().any(axis=1)==False]
    ...
    #-------------#
    # Get indices
    N_train_step = int(round(ks_input_data.shape[0]*Train_step_proportion,0))
    N_test_set = int(ks_input_data.shape[0] - round(ks_input_data.shape[0]*Train_step_proportion,0))
    # # Get Datasets
    X_train = ks_input_data[:N_train_step]
    X_test = ks_input_data[-N_test_set:]
    ...
    data_x_test = X_test
    # Get Targets 
    data_y = ks_output_data[:N_train_step]
    data_y_test = ks_output_data[-N_test_set:]
    ...
    print('ks_input_data shape:',ks_input_data.shape)
    #N_plot_ks = 2048
